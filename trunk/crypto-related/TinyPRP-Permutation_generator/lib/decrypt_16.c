...
static inline void mixcells_i_16(uint8_t a[2]);
static inline void shiftcells_i_16(uint8_t r[2]);
static inline void subbytes_i_16(uint8_t val[2]);
static inline void expandkey_16(uint16_t key[KEYSIZE_16], uint16_t *expendedkey);
...
static inline void InvSubBytes_i_16(uint8_t val[2]);
static inline void InvMixCells(uint8_t c[4]);
static inline void InvMixCells_i_16(uint8_t a[2]);
static inline void InvShiftCells_i_16(uint8_t r[2]);

static const uint8_t mod = 0x1B;
static const uint8_t bit7_shift = 7;
static const uint16_t rcon_i_16[RCON_COUNT_16] = {0x0000, 0x0101, 0x0202, 0x0404, 0x0808, 
                                            0x1010, 0x2020, 0x4040, 0x8080, 0x1B1B};
static const uint8_t c_i_16[] = {0x1, 0x1, 0x0, 0x0, 0x0, 0x1, 0x1, 0x0};
static const uint8_t Invc_i_16[] = {0x1, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0};
    ...
    for(i = 0; i < 2; i ++)
    {
        ...
        for(j = 0; j < 8; j ++)
        {
            bit = GETBIT(val[i], ((j + 2) % 8)) ^ GETBIT(val[i], ((j + 5) % 8)) ^ 
                  GETBIT(val[i], ((j + 7) % 8)) ^ Invc_i_16[j];
            SETBIT(ans, j, bit);
        }
        if(ans != 0x0)
        {
...
inline void ExpandKey_16(uint16_t key[KEYSIZE_16], uint16_t *expandedKey)
{
    ...
    for(i = 0; i < (KEYSIZE_16 + RCON_COUNT_16); i ++)
    {
        if(i < KEYSIZE_16)
        {
            expandedKey[i] = key[i];
        ...
        {
            expandedKey[i] = rcon_i_16[i - KEYSIZE_16];
            for(j = 0; j < i; j ++)
            {
                expandedKey[i] ^= expandedKey[j];
                SubBytes_i_16((uint8_t *)(expandedKey + i));
                ShiftCells_i_16((uint8_t *)(expandedKey + i));
                MixCells_i_16((uint8_t *)(expandedKey + i));
            }
        }
    }
}
...
void encrypt_16(uint16_t  *expandedKey, uint8_t a[2])
{
    ...
    *((uint16_t *)a) ^= expandedKey[0];
    ...
    for(i = 1; i < (KEYSIZE_16 + RCON_COUNT_16 - 1); i ++)
    {
        SubBytes_i_16(a);
        ShiftCells_i_16(a);
        MixCells_i_16(a);
...
void decrypt_16(uint16_t  *expandedKey, uint8_t a[4])
{
    int i;
    
    *((uint16_t *)a) ^= expandedKey[KEYSIZE_16 + RCON_COUNT_16 - 1];
    
    for(i = KEYSIZE_16 + RCON_COUNT_16 - 2; i > 0; i --)
    {
        InvShiftCells_i_16(a);
        InvSubBytes_i_16(a);
        *((uint16_t *)a) ^= expandedKey[i];
        InvMixCells_i_16(a);
    }
