...
void decrypt(uint32_t  *expandedKey, uint8_t a[4]);
static inline void InvSubBytes_i(uint8_t val[4]);
static inline void InvMixCells(uint8_t c[4]);
static inline void InvMixCells_i(uint8_t a[4]);
static inline void InvShiftCells_i(uint8_t r[4]);
...
void decrypt(uint32_t  *expandedKey, uint8_t a[4])
{
    ...
    *((uint32_t *)a) ^= expandedKey[KEYSIZE + RCON_COUNT - 1];
